...
public class InduRight  {
    ...
    public void runOpMode(HardwareMap hardwareMap, Telemetry telemetry) throws InterruptedException {
        ...
        // Noose.setPower(0);
            ...
            DT_power_0();
            reset_odo();
            while (!(left_back_drive.getCurrentPosition() <= -18000)) {
            ...
            DT_power_0();
            reset_odo();
            while (right_back_drive.getCurrentPosition() >= -37500) { // was 57500
                left_back_drive.setPower(0.3);
                left_front_drive.setPower(0.3);
                right_back_drive.setPower(0.3);
                right_front_drive.setPower(0.3);
            ...
            reset_odo();
            while (!(Noose.getCurrentPosition() <= -970)) { // was -970
                Noose.setPower(-1);
            }
            Noose.setPower(0);
            while (!(Slide.getCurrentPosition() >= 750)) {
                Slide.setPower(1);
            }
            Slide.setPower(0);
            while (!(Slide.getCurrentPosition() <= 700)) {
                Slide.setPower(-1);
            }
            Slide.setPower(0);
            sleep(500);
            ...
            arm.target = -3900;
            reset_odo();
            if (left_back_drive.getCurrentPosition() < -4000){
                left_back_drive.setPower(.5);
                left_front_drive.setPower(-.5);
                right_back_drive.setPower(-.5);
                right_front_drive.setPower(.5);
            }
            else {
                DT_power_0();
            }
            ...
            while (armOn == true){
                ...
                //   if (!(left_back_drive.getCurrentPosition() <= 1000)) {
                ...
                if (1 == 2){
                    ...
                    if (right_back_drive.getCurrentPosition() <= 18000) { //keep backing up until you're close
                        Noose.setTargetPosition((int) (FlipStep * 0.545 + knot));
                        left_back_drive.setPower(-0.2);
                        left_front_drive.setPower(-0.2);
                        right_back_drive.setPower(-0.2);
                        right_front_drive.setPower(-0.2);
                        telemetry.addData("hori odo", right_back_drive.getCurrentPosition());
                        telemetry.update();
                    }
                    else if (FlipStep < -3850){//when backed up far enough was 3900

                        Noose.setTargetPosition(-200);//deposit was -200

                        if (Noose.getCurrentPosition() < -100){
                            sleep(1000);
                            arm.target = 0;
                            if (!(left_back_drive.getCurrentPosition() >= 25000)) {
                                //  Noose.setTargetPosition((int) (FlipStep * 0.545 + knot));
                                left_back_drive.setPower(-0.5);
                                left_front_drive.setPower(0.5);
                                right_back_drive.setPower(0.5);

...
public class RedRight {
    ...
    public void runOpMode(HardwareMap hardwareMap, Telemetry telemetry) throws InterruptedException {
        ...
        if (1 == 1) {
            // Put run blocks here.
            telemetry.addData("No Side", "Detected");
            ...
            while (false == my_90complete) {//turn right 90
                imu_update();
                if (orientation.getYaw(AngleUnit.DEGREES) > 0 || orientation.getYaw(AngleUnit.DEGREES) >= -70) {
            ...
            reset_odo();
            //back past x
            while (right_back_drive.getCurrentPosition() < 37500) {
                left_back_drive.setPower(-0.3);
                left_front_drive.setPower(-0.3);
                right_back_drive.setPower(-0.3);
                right_front_drive.setPower(-0.3);
            ...
            reset_odo();
            while (!(left_back_drive.getCurrentPosition() >= 48000)) {
            ...
            Noose.setPower(0);
            while (!(Slide.getCurrentPosition() >= 650)) {
                Slide.setPower(0.5);
            }
            Slide.setPower(0);

            while (!(Slide.getCurrentPosition() <= 380)) {
                Slide.setPower(-0.5);
            }
            Slide.setPower(0);
            ...
            sleep(1000);
            intake.setPower(0);
            while (!(Noose.getCurrentPosition() >= 300)) {
                Noose.setPower(1);
            }
            Noose.setPower(0);
            ...
            //slight right strafe
            reset_odo();
            while (!(left_back_drive.getCurrentPosition() <= -14000)) {
                left_back_drive.setPower(0.5);
                left_front_drive.setPower(-0.5);
                right_back_drive.setPower(-0.5);
                right_front_drive.setPower(0.5);
            ...
            reset_odo();
            runTime.reset();
            while (right_back_drive.getCurrentPosition() < 28000 && runTime.seconds() < 3) {
                left_back_drive.setPower(-0.3);
                left_front_drive.setPower(-0.3);
                right_back_drive.setPower(-0.3);
                right_front_drive.setPower(-0.3);
